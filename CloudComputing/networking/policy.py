...
class AdaptivePolicy(object):
	...
	minUtilization = self.utilization.values()[0]
	minCore = self.utilization.keys()[0]
	#for core in self.topo.coreSwitches.keys():
	#    if minUtilization > self.utilization[core]:
	#        minUtilization = self.utilization[core]
	#	 minCore = core
	for core, utilization in self.utilization.iteritems():
