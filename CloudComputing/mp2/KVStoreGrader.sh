...
then 
	...
	echo "SBS Create Test error, expected count: ${expected_count}, actual count: ${create_success_count}"
else
	...
	do 
		...
		then
			CREATE_TEST_STATUS="${FAILURE}"
			echo "SBS Create Test error, key: ${key}, count: ${key_create_success_count}"
			break
		fi
	done
fi
...
then
	...
	echo "Read test 2 failed. Expected success count: ${QUORUMPLUSONE}, actually got: ${read_test2_success_count}"
fi
...
then
	...
	echo "Read test 5 failed. Expected fail count: ${QUORUMPLUSONE} or ${RFPLUSONE}, actually got: ${read_test5_fail_count}"
fi
